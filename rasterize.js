...
if (system.args.length < 3 || system.args.length > 7) {
  console.log(
    "Usage: rasterize.js URL filename paperwidth*paperheight|paperformat portrait|landscape margin zoomfactor"
  );
  console.log(
    '  paperwidth*paperheight|paperformat examples: "5in*7.5in", "10cm*20cm", "A4", "Letter"'
  );
  ...
  page.open(address, function(status) {
    if (status !== "success") {
      console.error(
        "Unable to load the address (" + statusCode + "): " + address,
        status
      );
      ...
      if (hasHeaderOrFooter) {
        ...
        var typeOfHeight = page.evaluate(function(headerOrFooter) {
          return (
            ...
            typeof html2pdf[headerOrFooter].height
          );
        }, headerOrFooter);
        ...
        var typeOfContent = page.evaluate(function(headerOrFooter) {
          return (
            html2pdf[headerOrFooter].contents &&
            typeof html2pdf[headerOrFooter].contents
          );
        }, headerOrFooter);
    ...
    function getHtmlWithStyles(headerOrFooter, pageNumber, totalPages) {
      return page.evaluate(
        function(headerOrFooter, pageNumber, totalPages) {
